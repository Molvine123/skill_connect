...
<div class="animate-fade-up" style="max-width:860px;margin:0 auto;">
    ...
    <div class="table-container" style="padding:2rem;">
        <div style="margin-bottom:2rem;">
            <h2 style="font-size:1.25rem;font-weight:700;">Edit Vacancy: {{ $job->title }}</h2>
            <p style="color:#6b7280;font-size:.9rem;margin-top:.25rem;">Update the details of your job or internship posting.</p>
        </div>
        ...
        <form method="POST" action="{{ route('employer.jobs.update', $job->id) }}">
            ...
            <div style="margin-bottom:1.5rem;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-top:.5rem;">
                    <label style="display:flex;align-items:center;gap:.75rem;padding:1rem;border:2px solid {{ old('type',$job->type)==='job'?'#0d9488':'var(--sc-dark-border)' }};border-radius:10px;cursor:pointer;transition:.2s;" id="typeJobCard">
                        <input type="radio" name="type" value="job" {{ old('type',$job->type)==='job'?'checked':'' }} style="accent-color:#0d9488;" onchange="updateTypeCard()">
                        <div>
                            <div style="font-weight:700;">💼 Job</div>
                            <div style="font-size:.8rem;color:#6b7280;">Permanent or contract employment</div>
                        </div>
                    </label>
                    <label style="display:flex;align-items:center;gap:.75rem;padding:1rem;border:2px solid {{ old('type',$job->type)==='internship'?'#7c3aed':'var(--sc-dark-border)' }};border-radius:10px;cursor:pointer;transition:.2s;" id="typeInternCard">
                        <input type="radio" name="type" value="internship" {{ old('type',$job->type)==='internship'?'checked':'' }} style="accent-color:#7c3aed;" onchange="updateTypeCard()">
                        <div>
            ...
            <div style="margin-bottom:1.25rem;">
                ...
                <textarea name="description" rows="5" class="form-input" required>{{ old('description', $job->description) }}</textarea>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;margin-bottom:1.25rem;">
                ...
                <div>
                    ...
                    <select name="employment_type" class="form-input">
                        ...
                        <option value="{{ $t }}" {{ old('employment_type', $job->employment_type)===$t?'selected':'' }}>{{ $t }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;margin-bottom:1.25rem;">
                ...
                <div>
                    ...
                    <input type="text" name="salary" value="{{ old('salary', $job->salary) }}" class="form-input">
                </div>
                {{-- Duration (internship) --}}
                <div>
                    <label class="form-label">Duration (for Internships)</label>
                    <input type="text" name="duration" value="{{ old('duration', $job->duration) }}" class="form-input">
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1.25rem;margin-bottom:1.25rem;">
                ...
                <div>
                    ...
                    <select name="experience_level" class="form-input">
                        ...
                        <option value="{{ $lvl }}" {{ old('experience_level', $job->experience_level)===$lvl?'selected':'' }}>{{ $lvl }} Level</option>
                        @endforeach
                    </select>
                </div>
                {{-- Deadline --}}
                <div>
                    ...
                    <input type="date" name="deadline" value="{{ old('deadline', $job->deadline ? $job->deadline->format('Y-m-d') : '') }}" class="form-input">
            ...
            <div style="margin-bottom:1.25rem;">
                ...
                <input type="text" name="required_skills" value="{{ old('required_skills', $job->required_skills) }}" class="form-input">
            ...
            <div style="margin-bottom:1.25rem;">
                ...
                <input type="text" name="required_qualifications" value="{{ old('required_qualifications', $job->required_qualifications) }}" class="form-input">
            </div>

            {{-- Requirements --}}
            <div style="margin-bottom:2rem;">
                <label class="form-label">Additional Requirements</label>
                <textarea name="requirements" rows="3" class="form-input">{{ old('requirements', $job->requirements) }}</textarea>
            </div>
